...
void mbm_gps_ni_request(GpsCtrlSuplNiRequest *ni_request)
{
    ...
    MBMLOGD("%s: notification filled with id:%d, type:%d, id_encoding:%d, text_encoding:%d", 
            __FUNCTION__, context->notification.notification_id, context->notification.ni_type,
            context->notification.requestor_id_encoding, context->notification.text_encoding);
...
retry:
        ...
        do {
            ...
            if (at_dev || nmea_dev) {
                MBMLOGI("Waiting for %s%s%s to be removed (%d)...",
                          ...
                          uint32_t preferred_time)
{
    ...
    MBMLOGD("%s:enter  %s min_interval = %d recurrence=%d pref_time=%d",
